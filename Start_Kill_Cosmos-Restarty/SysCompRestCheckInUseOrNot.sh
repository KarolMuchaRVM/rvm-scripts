#!/bin/bash
#
# Execute "system_complete_restart" if the machine is not in use
...
while true; do
  ...
  if $MACHINE_IN_USE ; then
    ...
    echo "execute system_complete_restart $DATE"
    echo "execute system_complete_restart  $DATE" >> /home/repant/COSMOS/outbox/feedback.txt
    sh /home/repant/COSMOS/script/system_complete_restart
    break
  fi
done
exit 0
